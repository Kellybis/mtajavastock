...
import kelly.org.dto.NasdaqSymbolDto;
...
public class MarketService {
	...
	private final String YQL_QUERY = "select * from yahoo.finance.quotes where symbol in ("
			+ $_SYMBOLS + ")";
	...
	public List<Stock> getStocks(List<String> symbols)
		...
		for (int i = 0; i < symbols.size(); i++) {
			...
			// on batch size limit - send an HTTP request to fetch next symbols
			// batch.
	...
	private List<Stock> doCall(StringBuilder sb) throws SymbolNotFoundInNasdaq {
		...
		try {
			...
			if (jsonSB.length() > 0) {
				...
				// Yahoo result is array for more than 1 result and an object to
				// a single result.
				if (count > 1) {
					...
					for (int j = 0; j < arr.length(); j++) {
						...
						if (!quote
								.isNull("ErrorIndicationreturnedforsymbolchangedinvalid")) {
							log.warning("someone tried adding stock which doesn't exists: "
									+ sb.toString());
							throw new SymbolNotFoundInNasdaq(
		...
		if (this.nasdaqSymbols == null || this.nasdaqSymbols.isEmpty()) {
			...
			try {
				...
				while ((line = rd.readLine()) != null) {
					...
					this.nasdaqSymbols.add(new NasdaqSymbolDto(symbol, company,
							industry));
				}

				log.log(Level.INFO, "Finished fetching symboles.Found "
						+ this.nasdaqSymbols.size() + " symbols.");
	...
	private Stock fromJson(JSONObject stockJson) {
		try {
			...
			if (!stockJson.isNull("Bid"))
				ret.setBid((float) stockJson.getDouble("Bid"));
